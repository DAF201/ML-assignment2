...
class FFNN(nn.Module):
    ...
    def compute_Loss(
        self, predicted_vector, gold_label
    ):  # computes the loss between predicted vector and gold(true) label
...
def convert_to_vector_representation(data, word2index):
    ...
    for document, y in data:
        ...
        for word in document:
            ...
            vector[index] += 1
        vectorized_data.append((vector, torch.tensor(y)))
    ...
    for epoch in range(args.epochs):
        ...
        for minibatch_index in tqdm(range(N // minibatch_size)):
            ...
            for example_index in range(minibatch_size):
                ...
                total += 1
                gold_label = torch.tensor([gold_label]).to("cuda")
                example_loss = model.compute_Loss(
                    predicted_vector.view(1, -1), gold_label
                )
                if loss is None:
                    loss = example_loss
                else:
...
cost = datetime.now().timestamp() - st.timestamp()

print(
    "time cost for %d epoches:" % args.epochs,
    cost,
    " avg: ",
    (cost / args.epochs),
)
print(
    "time cost for %d hidden_dim:" % args.hidden_dim,
    cost,
    " avg: ",
    (cost / args.hidden_dim),
)
